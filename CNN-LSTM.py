...
def cross_validate(model_fn, dataset, config, class_names):
    ...
    try:
        disp = ConfusionMatrixDisplay.from_predictions(y_true_all, y_pred_all, display_labels=class_names, cmap='Blues')
...
def main():
    ...
    le = LabelEncoder()
    y_encoded = le.fit_transform(labels_order)
    ...
    try:
        ...
        acc_mean, acc_std = calculate_mean_and_std(val_accs)

        epochs = range(1, len(train_mean) + 1)
        fig, ax1 = plt.subplots(figsize=(12, 8))
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Loss')
        ax1.plot(epochs, train_mean, 'skyblue', label='Train Loss')
        ax1.fill_between(epochs, train_mean - train_std, train_mean + train_std, alpha=0.2)
        ax1.plot(epochs, val_mean, 'lightgreen', label='Val Loss')
        ax1.fill_between(epochs, val_mean - val_std, val_mean + val_std, alpha=0.2)

        ax2 = ax1.twinx()
        ax2.set_ylabel('Accuracy')
        ax2.plot(epochs, acc_mean, 'salmon', label='Val Acc')
        ax2.fill_between(epochs, acc_mean - acc_std, acc_mean + acc_std, alpha=0.2)
        ax2.set_ylim(0, 1)
